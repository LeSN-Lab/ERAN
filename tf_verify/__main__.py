...
if dataset=='acasxu':
    ...
    for i, test in enumerate(tests):
        ...
        if domain == 'gpupoly' or domain == 'refinegpupoly':
            ...
            printsave(nlb[-1])
            #################################################
            if label == int(test[0]): #라벨이 같을 떄, 옮게 correctly classified 되었을 떄.
        ...
        if is_correctly_classified == True:
            ...
            if domain == 'gpupoly' or domain =='refinegpupoly':
                ...
                if domain.endswith("poly"):
                    ...
                    printsave("nlb ", nlb[-1])                    
                    printsave("nub ", nub[-1])
                    printsave("adv labels ", failed_labels)
                    ###################################################################################################                    
...
########################################################################################################################
                                    printsave("img ", i, " Verified unsage against label ", cex_label, " correct label ", label)
########################################################################################################################
                                    unsafe_images+=1
                                else:
